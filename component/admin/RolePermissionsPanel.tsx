...
import PolicyDefinitionsModal from "./PolicyDefinitionsModal";
...
export default function RolePermissionsPanel({
  ...
  const handleViewDefinition = async (permission: string) => {
    try {
      const res = await axios.get<PolicyDefinition[]>(
        `${ADMIN_API}/policies/${encodeURIComponent(permission)}/definitions`
      );
      setViewDefinitionsFor({ permission, definitions: res.data });
    } catch (err) {
              ...
              <tbody className="divide-y divide-slate-100">
                {permissions.map((perm, idx) => (
                  <tr key={perm.permission} className="hover:bg-slate-50">
                    ...
                    <td className="py-3 pr-4 text-slate-700">{perm.page}</td>
                    ...
                    <td className="py-3 pr-2">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => handleViewDefinition(perm.permission)}
